...
const adicionaParametrosNoInput = element => {
  ...
  if (hora.minutos < 10) inputHora.value = `Tempo ${hora.hora}:0${hora.minutos}`
...
const calculaHora = tempoEmMilessegundos => {
  const tempo = {
    horas: +(tempoEmMilessegundos / 3600000).toFixed(0),
    minutos: +((tempoEmMilessegundos / 60000) % 60).toFixed(0)
  }
